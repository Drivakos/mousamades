<template>
  <div class="build-with-outer-wrapper parallax-services">
    <div class="build-with-title">ΥΛΙΚΑ</div>
    <div class="build-with-wrapper">
      <a href="https://www.mehler-texnologies.com/en/produkte-archive/tarp-fr/" class="slide">
        <img src="/build-with-assets/melher-logo.png" alt="Mousamades Drivakos logo" class="site-logo">
        <div class="slide-text">ΥΛΙΚΑ <br> ΜΟΥΣΑΜΑΔΩΝ</div>
      </a>
      <a class="slide">
        <img src="/build-with-assets/stickers.png" alt="Mousamades Drivakos logo" class="site-logo">
        <div class="slide-text"> ΚΟΠΤΙΚΑ <br> ΑΥΤΟΚΟΛΛΗΤΑ</div>
      </a>
    </div>
  </div>
</template>

<script>

</script>
